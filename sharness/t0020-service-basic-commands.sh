...
test_expect_success "custer-service help output looks good" '
    rep-mgr-service --help | egrep -q -i "^(Usage|Commands|Description|Global Options)"
'

test_expect_success "cluster-service --version succeeds and matches ctl" '
    export SERV_VERSION=`rep-mgr-service --version | grep -Po "\d+\.\d+\.\d+"`
    export CTL_VERSION=`rep-mgr-ctl --version | grep -Po "\d+\.\d+\.\d+"`
    [ "$SERV_VERSION" = "$CTL_VERSION" ]
'

test_expect_success "starting a second cluster-service process fails" '
    test_expect_code 1 rep-mgr-service --config "test-config"
'
