...
test_expect_success IPFS,CLUSTER "add files locally and compare with ipfs" '
    ...
    rep-mgr-ctl add  smallfile.bin > cidscluster.txt
    rep-mgr-ctl add  -w smallfile.bin >> cidscluster.txt

    rep-mgr-ctl add  --raw-leaves -w smallfile.bin >> cidscluster.txt
    rep-mgr-ctl add  --raw-leaves smallfile.bin >> cidscluster.txt

    rep-mgr-ctl add  bigfile.bin >> cidscluster.txt
    rep-mgr-ctl add  --layout trickle bigfile.bin >> cidscluster.txt
    rep-mgr-ctl add  -w bigfile.bin >> cidscluster.txt
    rep-mgr-ctl add  --raw-leaves -w bigfile.bin >> cidscluster.txt
    rep-mgr-ctl add  --raw-leaves bigfile.bin >> cidscluster.txt

    rep-mgr-ctl add -r testFolder >> cidscluster.txt
    rep-mgr-ctl add -r -w testFolder >> cidscluster.txt

    rep-mgr-ctl add --cid-version 1 -r testFolder >> cidscluster.txt
    rep-mgr-ctl add --hash sha3-512 -r testFolder >> cidscluster.txt
...
test_expect_success IPFS,CLUSTER "add CAR file" '
    ...
    rep-mgr-ctl add --format car -Q test.car >> carcluster.txt
...
test_expect_success IPFS,CLUSTER "check add folders" '
    ...
    rep-mgr-ctl add --quieter -w testFolder2/abc.txt > wrapped.txt
    rep-mgr-ctl add --quieter -r testFolder2 > folder.txt
...
test_expect_success IPFS,CLUSTER "check pin after locally added" '
    ...
    cid=`rep-mgr-ctl add -r --quieter testFolder3`
    rep-mgr-ctl pin ls | grep -q -i "$cid"
'

test_expect_success IPFS,CLUSTER "add with metadata" '
    echo "test1" > test1.txt
    cid1=`rep-mgr-ctl add --quieter --metadata kind=text test1.txt`
    echo "test2" > test2.txt
    cid2=`rep-mgr-ctl add --quieter test2.txt`
    rep-mgr-ctl pin ls "$cid1" | grep -q "Metadata: yes" &&
    rep-mgr-ctl --enc=json pin ls "$cid1" | jq .metadata | grep -q "\"kind\": \"text\"" &&
    rep-mgr-ctl pin ls "$cid2" | grep -q "Metadata: no"
'
