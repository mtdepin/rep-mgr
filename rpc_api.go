...
func (rpcapi *ClusterRPCAPI) BlockAllocate(ctx context.Context, in api.Pin, out *[]peer.ID) error {
	...
	allocs, err := rpcapi.c.allocate(
		...
		[]peer.ID{},                            // blacklist
		api.StringsToPeers(in.UserAllocations), // prio list
	)
...
func (rpcapi *ClusterRPCAPI) GetNodesForCid(ctx context.Context, cids []api.Cid, out *map[api.Cid][]api.Node) error {
	...
	*out = make(map[api.Cid][]api.Node)
	metrics := rpcapi.c.monitor.LatestMetrics(ctx, "freespace")
	if len(metrics) == 0 {
		return nil
	}
	maxCount := 20

	fillter := func(alloc string, cid string) api.Node {
		for _, metric := range metrics {
			for _, knode := range metric.KnodeList {
				if !metric.Valid {
					continue
				}

				if !strings.Contains(alloc, knode.PeerID.String()) {
					continue
				}

				n := api.Node{
					Address: fmt.Sprintf("%s/get?cid=%s", knode.Address, cid),
					Weight:  0,
				}
				return n
			}
	...
	for _, cid := range cids {
		pin := rpcapi.c.tracker.GetPinFromState(ctx, cid)
		if pin.Allocations == nil || len(pin.Allocations) == 0 {
			continue
		}
		nodes := make([]api.Node, 0)
		for _, allocation := range pin.Allocations {
			_, kId, _ := api.ParseAllocate(allocation)
			node := fillter(kId.String(), cid.String())
...
func (rpcapi *ClusterRPCAPI) GetKnodeList(ctx context.Context, args map[string]string, out *api.KNodeAddrs) error {
	...
	if len(metrics) > 0 {
		//todo :通过args条件过滤和数量
		var targets = make([]string, 60)
		var extend = make([]string, 60)
		count := 0
		for _, metric := range metrics {
			for _, knode := range metric.KnodeList {
				if !metric.Valid {
				...
				targets[count] = fmt.Sprintf("%s/add", knode.Address)
				//targets[count] = fmt.Sprintf("%s", knode.Address)
				extend[count] = knode.String()
				count++
				if count >= maxCount {
...
func (rpcapi *IPFSConnectorRPCAPI) Unpin(ctx context.Context, in api.Pin, out *struct{}) error {
	return rpcapi.ipfs.Unpin(ctx, in)
}
...
func (rpcapi *IPFSConnectorRPCAPI) PinLs(ctx context.Context, in <-chan []string, out chan<- api.IPFSPinInfo) error {
	select {
	case <-ctx.Done():
		logger.Info("PinLs...., context have been done")
		close(out)
		return ctx.Err()
	case pinTypes, ok := <-in:
		if !ok {
