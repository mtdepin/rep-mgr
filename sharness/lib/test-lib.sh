...
# Sharness test framework for rep-mgr
#
...
test_cluster_init() {
    ...
    which rep-mgr-service >/dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "cluster init FAIL: rep-mgr-service not found"
        exit 1
    fi
    which rep-mgr-ctl >/dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "cluster init FAIL: rep-mgr-ctl not found"
        exit 1
    fi
    rep-mgr-service --config "test-config" init --force --consensus "$consensus" >"$IPFS_OUTPUT" 2>&1
    if [ $? -ne 0 ]; then
...
cluster_kill(){
    pkill -1 -f rep-mgr-service
    while pgrep rep-mgr-service >/dev/null; do
        sleep 0.2
    done
}

cluster_start(){
    rep-mgr-service --config "test-config" daemon >"$IPFS_OUTPUT" 2>&1 &
