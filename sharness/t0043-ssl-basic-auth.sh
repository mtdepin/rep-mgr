...
test_expect_success "ssl interaction fails with bad credentials" '
    id=`cluster_id`
    { test_must_fail rep-mgr-ctl --no-check-certificate --basic-auth "testuser:badpass" id; } | grep -A1 "401" | grep -i "unauthorized"
'

test_expect_success "ssl interaction succeeds" '
    id=`cluster_id`
    rep-mgr-ctl --no-check-certificate --basic-auth "testuser:testpass" id | egrep -q "$id"
'
