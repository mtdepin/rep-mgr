...
import (
	...
	"github.com/mtdepin/rep-mgr/adder"
	"github.com/mtdepin/rep-mgr/adder/sharding"
	"github.com/mtdepin/rep-mgr/adder/single"
	"github.com/mtdepin/rep-mgr/api"
	"github.com/mtdepin/rep-mgr/pstoremgr"
...
const (
	...
	mdnsServiceTag      = "_rep-mgr-discovery._udp"
	maxAlerts           = 1000
)
...
func NewCluster(
	...
	logger.Infof("Replication Manager v%s listening on:\n%s\n", version.Version, listenAddrs)
...
func (c *Cluster) watchPinset() {
	...
	recoverTimer := time.NewTimer(180 * time.Second) // 0 so that it does an initial recover right away
...
func (c *Cluster) alertsHandler() {
	for {
		...
		case alrt := <-c.monitor.Alerts():
			...
			for pin := range pinCh {
				if containsPeer(api.StringsToPeers(pin.Allocations), alrt.Peer) && distance.isClosest(pin.Cid) {
					c.repinFromPeer(c.ctx, alrt.Peer, pin)
				}
			}
		case peerAlert := <-c.ipfs.Alerts():
			logger.Info("===> handle peer alert")
			//handle peer alert
			logger.Info(peerAlert)
			...
			distance, err := c.distances(c.ctx, peerAlert.Peer)
			if err != nil {
				logger.Warn(err)
				return
			}
			...
			for pin := range pinCh {
				if containsPeer(api.StringsToPeers(pin.Allocations), peerAlert.Peer) && distance.isClosest(pin.Cid) {
					c.repinFromPeer(c.ctx, peerAlert.Peer, pin)
				}
			}
		}
...
func (c *Cluster) vacatePeer(ctx context.Context, p peer.ID) {
	...
	for pin := range pinCh {
		if containsPeer(api.StringsToPeers(pin.Allocations), p) {
...
func (c *Cluster) repinFromPeer(ctx context.Context, p peer.ID, pin api.Pin) {
	...
	logger.Warnf("repinning %s from peer %s", pin.Cid, p)
...
func (c *Cluster) ready(timeout time.Duration) {
	...
	select {
	case <-timer.C:
		logger.Error("***** rep-mgr consensus start timed out (tips below) *****")
		logger.Error(`
**************************************************
...
`)
	...
	// Wait for ipfs
	logger.Info("Waiting for IPFS manager to be ready...")
	select {
	case <-ctx.Done():
		return
	case <-c.ipfs.Ready(ctx):
		...
		if err != nil {
			logger.Error("IPFS manager signaled ready but ID() errored: ", err)
		} else {
			logger.Infof("IPFS manager is ready. Peer ID: %s", ipfsid.ID)
		}
	}
	...
	logger.Info("** Replication manager  is READY **")
}
...
func (c *Cluster) setupPin(ctx context.Context, pin, existing api.Pin) (api.Pin, error) {
	...
	//pin.Metadata = c.mergePinMeta(ctx, pin, existing)
	return pin, checkPinType(pin)
}

func (c *Cluster) mergePinMeta(ctx context.Context, pin, existing api.Pin) map[string]string {
...
func (c *Cluster) pin(
	...
	//pin.Allocations = []string{
	//	"12D3KooWHiXbnwUpnh7wzPX2vHaLq9QdQHpzSuYmM9PB97RFSSc7.12D3KooWAPUMWUtYS9fMnKYfA5JcXPtN2UMAhvTXb6MKkvD4ov2W",
	//	"12D3KooWHiXbnwUpnh7wzPX2vHaLq9QdQHpzSuYmM9PB97RFSSc7.12D3KooWB8E1EnoXTPEiFsmNTRYFMwCnJkd2mqXJbDrrG3h2T6nA",
	//	//"12D3KooWHiXbnwUpnh7wzPX2vHaLq9QdQHpzSuYmM9PB97RFSSc7.12D3KooWK5auMDP3bR4xfCNuGCKVNG27gpKvPkQ8eU5sstSwe9PP",
	//}
	...
	if len(pin.Allocations) == 0 {
		...
		allocs, err := c.allocate(
			...
			blacklist,
			api.StringsToPeers(pin.UserAllocations),
		)
		if err != nil {
			return pin, false, err
		}
		newAlloc := api.PeersToStrings(allocs)
		pin.Allocations = newAlloc
	}

	// If this is true, replication factor should be -1.
	if len(pin.Allocations) == 0 {
		logger.Infof("pinning %s everywhere:", pin.Cid)
		return pin, false, errors.New("not support everywhere")
	} else {
...
func (c *Cluster) globalPinInfoCid(ctx context.Context, comp, method string, h api.Cid) (api.GlobalPinInfo, error) {
	...
	if c.config.FollowerMode {
		...
		if !pin.IsPinEverywhere() {
			dests = api.StringsToPeers(pin.Allocations)
			dests, err = api.ParseRmPeersFromAllocator(dests)
			if err != nil {
				logger.Error(err)
				return gpin, err
			}
