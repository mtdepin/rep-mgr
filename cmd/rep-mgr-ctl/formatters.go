...
func textFormatPrintPin(obj api.Pin) {
	...
	if len(obj.Origins) > 0 {
		//id, err := obj.Origins[0].ValueForProtocol(multiaddr.P_P2P)
		//if err != nil {
		//	logger.Error(err)
		//	origin = obj.Origins[0].String()
		//}

		//arr := strings.Split(obj.Origins[0].String(), "/")
		//if len(arr) > 0 {
		//	origin = arr[len(arr)-1]
		//}
		origin = obj.Origins[0].String()
	}
