...
import (
	...
	state "github.com/mtdepin/rep-mgr/state"
...
func NewAPIWithHost(ctx context.Context, cfg *Config, h host.Host, routes func(*rpc.Client) []Route) (*API, error) {
	...
	router = router.PathPrefix("/api/" + ApiVersion).Subrouter()
	...
	// See: https://github.com/mtdepin/rep-mgr/issues/548
...
func (api *API) setupLibp2p() error {
	...
	if len(api.config.Libp2pListenAddr) > 0 {
		...
		// https://github.com/mtdepin/rep-mgr/issues/853
		h, err := libp2p.New(
