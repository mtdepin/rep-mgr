...
func (ipfs *Connector) Alerts() <-chan api.Alert {
	//TODO implement me
	panic("implement me")
}
...
func (ipfs *Connector) Pin(ctx context.Context, pin api.Pin) error {
	...
	if pinStatus.IPFSPinStatus.IsPinned(maxDepth) {
	...
	if from := pin.PinUpdate; from.Defined() {
		...
		if pinStatus.IPFSPinStatus.IsPinned(-1) { // pinned recursively.
...
// daemon.
func (ipfs *Connector) Unpin(ctx context.Context, pin api.Pin) error {
	hash := pin.Cid
...
func (ipfs *Connector) PinLsCid(ctx context.Context, pin api.Pin) (api.PinDetail, error) {
	return api.PinDetail{}, nil
}
...
//	defer span.End()
//
//	ctx, cancel := context.WithTimeout(ctx, ipfs.config.KnodeRequestTimeout)
//	defer cancel()
//
//	if !pin.Defined() {
