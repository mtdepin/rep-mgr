...
import (
	...
	types "github.com/mtdepin/rep-mgr/api"
	"github.com/mtdepin/rep-mgr/api/common"
...
func (api *API) routes(c *rpc.Client) []common.Route {
	...
	return []common.Route{
		...
		},
		{
			...
			Method:      "GET",
			Pattern:     "/status/{cids}",
			HandlerFunc: api.pinDetailHandler,
		},
		{
			Name:        "Pin",
			Method:      "POST",
		...
		},
		{
			...
			Method:      "GET",
			Pattern:     "/nodelist",
			HandlerFunc: api.GetKnodeList,
		},
		{
			Name:        "GetKnodeList",
			Method:      "GET",
			Pattern:     "/nodes/{cids}",
			HandlerFunc: api.GetNodesForCid,
		},
	}
}
...
func (api *API) addHandler(w http.ResponseWriter, r *http.Request) {
	...
	api.SendResponse(w, common.SetStatusAutomatically, errors.New("add not support"), nil)
...
func (api *API) pinHandler(w http.ResponseWriter, r *http.Request) {
	if pin := api.ParseCidOrFail(w, r); pin.Defined() {
		...
		//maitian : change by lyc 加入status字段
		result := struct {
			types.Pin
			Status int
		}{
		...
		api.SendResponse(w, common.SetStatusAutomatically, err, result)
		api.config.Logger.Debug("rest api pinHandler done")
	}
}
...
func (api *API) statusHandler(w http.ResponseWriter, r *http.Request) {
	...
	if pin := api.ParseCidOrFail(w, r); pin.Defined() {
		if local == "true" {
			...
			globalPinInfo := pinInfo.ToGlobal()

			if err != nil {
				api.SendResponse(w, common.SetStatusAutomatically, err, globalPinInfo)
				return
			}
...
func (api *API) pinDetailHandler(w http.ResponseWriter, r *http.Request) {

	vars := mux.Vars(r)
	cidStr := vars["cids"]
	cids := strings.Split(cidStr, ",")
	resultMap := make(map[types.Cid]PinStatus)
	for _, cid := range cids {
		c, err := types.DecodeCid(cid)
		if err != nil {
			logger.Error(err)
			api.SendResponse(w, http.StatusBadRequest, errors.New("error decoding Cid: "+err.Error()), nil)
			return
		}
		pin := types.PinWithOpts(c, types.PinOptions{})
		if pin.Defined() {
			var pinInfo types.GlobalPinInfo
			err := api.rpcClient.CallContext(
				r.Context(),
			...
			if err != nil {
				...
				for _, pi := range pinInfo.PeerMap {
					for _, ps := range pi {
						if ps.IsPined() {
							status.PinCount = status.PinCount + 1
						}
					}
				}
			}

			resultMap[pin.Cid] = status
		}
	}
	...
	api.SendResponse(w, common.SetStatusAutomatically, nil, result)
}
...
func (api *API) GetNodesForCid(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	cidStr := vars["cids"]
	cidArr := strings.Split(cidStr, ",")
	cids := make([]types.Cid, len(cidArr))
	...
	type Result struct {
		Data   map[types.Cid][]types.Node
		Status int
	}
	var data map[types.Cid][]types.Node
	err := api.rpcClient.CallContext(
		r.Context(),
		"",
		"Cluster",
	...
	if err != nil {
		logger.Error(err)
		api.SendResponse(w, http.StatusBadRequest, errors.New("error : "+err.Error()), nil)
		return
	}
	...
	api.SendResponse(w, common.SetStatusAutomatically, err, out)

}
func (api *API) GetKnodeList(w http.ResponseWriter, r *http.Request) {
	var result types.KNodeAddrs
	...
	result.Count = len(result.Extend)
	result.Status = 1
	api.SendResponse(w, common.SetStatusAutomatically, err, result)
}
