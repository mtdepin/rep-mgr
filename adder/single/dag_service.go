...
import (
	...
	adder "github.com/mtdepin/rep-mgr/adder"
...
func (dgs *DAGService) Add(ctx context.Context, node ipld.Node) error {
	...
	if dgs.dests == nil {
		...
		for i, d := range dests {
			if d == string(localPid) || d == "" {
				hasLocal = true
				// ensure our allocs do not carry an empty peer
				// mostly an issue with testing mocks
				dests[i] = string(localPid)
			}
		}

		dgs.dests = api.StringsToPeers(dests)
...
func (dgs *DAGService) Finalize(ctx context.Context, root api.Cid) (api.Cid, error) {
	...
	rootPin.Allocations = api.PeersToStrings(dgs.dests)
