...
func (c *Cluster) allocate(ctx context.Context, hash api.Cid, currentPin api.Pin, rplMin, rplMax int, blacklist []peer.ID, priorityList []peer.ID) ([]peer.ID, error) {
	...
	if currentPin.Defined() {
		currentAllocs = api.StringsToPeers(currentPin.Allocations)
	}
...
func (c *Cluster) filterMetrics(ctx context.Context, mSet api.MetricsSet, numMetrics int, currentAllocs, priorityList, blacklist []peer.ID) classifiedMetrics {
	...
	fillMetricsSet := func(peersMap map[peer.ID][]api.Metric) (api.MetricsSet, []peer.ID) {
		...
		for p, metrics := range peersMap {
			logger.Infof("peerid: ", p)
			if len(metrics) == numMetrics {
				for _, m := range metrics {
					//mSet[m.Name] = append(mSet[m.Name], m)
					if m.KnodeList != nil {
						for _, kmetric := range m.KnodeList {
							tm := kmetric.ToMetric(m)
							mSet[m.Name] = append(mSet[m.Name], tm)
							peers = append(peers, tm.Peer)
						}
					}
				}
...
func (c *Cluster) filterMetrics_2(ctx context.Context, mSet api.MetricsSet, numMetrics int, currentAllocs, priorityList, blacklist []peer.ID) classifiedMetrics {
	classified := classifiedMetrics{
		current:        make(api.MetricsSet),
		currentPeers:   make([]peer.ID, 0),
		candidate:      make(api.MetricsSet),
		candidatePeers: make([]peer.ID, 0),
		priority:       make(api.MetricsSet),
		priorityPeers:  make([]peer.ID, 0),
	}
		...
		for _, m := range metrics {
			...
			default:
				for _, kmetric := range m.KnodeList {
					tm := kmetric.ToMetric(m)
					//check is current ?
					if containsPeer(currentAllocs, tm.Peer) {
						//current
						classified.current[m.Name] = append(classified.current[m.Name], tm)
						classified.currentPeers = append(classified.currentPeers, tm.Peer)

					} else if containsPeer(priorityList, m.Peer) || containsPeer(priorityList, tm.Peer) {
						//priority
						classified.priority[m.Name] = append(classified.priority[m.Name], tm)
						classified.priorityPeers = append(classified.priorityPeers, tm.Peer)

					} else {
						//candidate
						classified.candidate[m.Name] = append(classified.candidate[m.Name], tm)
						classified.candidatePeers = append(classified.candidatePeers, tm.Peer)
					}
...
func (c *Cluster) obtainAllocations(
	...
	logger.Infof("obtainAllocations: current: %d", nCurrentValid)
	logger.Infof("obtainAllocations: available: %d", nAvailableValid)
	logger.Infof("obtainAllocations: candidates: %d", len(metrics.candidatePeers))
	logger.Infof("obtainAllocations: priority: %d", len(metrics.priorityPeers))
	logger.Infof("obtainAllocations: Needed: %d", needed)
	logger.Infof("obtainAllocations: Wanted: %d", wanted)
