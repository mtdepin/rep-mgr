...
test_expect_success "cluster-service init with --peers succeeds and fills peerstore" '
    ...
    rep-mgr-service --config "test-config" init --peers $PEER1,$PEER2 &&
...
test_expect_success "cluster-service init without --peers succeeds and creates empty peerstore" '
    rep-mgr-service --config "test-config" init -f &&
...
test_expect_success "cluster-service init with raft generates only raft config" '
    rep-mgr-service --config "test-config" init -f --consensus raft &&
...
test_expect_success "cluster-service init with crdt generates only crdt config" '
    rep-mgr-service --config "test-config" init -f --consensus crdt &&
    [ "$(jq -M -r .consensus.crdt test-config/service.json)" != "null" ] && 
    [ "$(jq -M -r .consensus.raft test-config/service.json)" == "null" ]
'
