			...
			*/
			...
			Action: func(c *cli.Context) error {
				...
				if c.String("allocations") != "" {
					p.UserAllocations = strings.Split(c.String("allocations"), ",")
				}
...
`,
		...
		},
		{
			...
			Hidden:    true,
			Action: func(c *cli.Context) error {
				walkCommands(c.App.Commands, "rep-mgr-ctl")
				return nil
			},
		},
	}
