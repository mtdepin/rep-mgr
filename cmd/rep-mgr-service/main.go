...
const programName = "rep-mgr-service"
...
https://github.com/mtdepin/rep-mgr.
...
$ rep-mgr-service init

Launch a cluster:

$ rep-mgr-service daemon

Launch a peer and join existing cluster:

$ rep-mgr-service daemon --bootstrap /ip4/192.168.1.2/tcp/9096/p2p/QmPSoSaPXpyunaBwHs1rZBKYSqRV4bLRk32VGYLuvdrypL
...
$ rep-mgr-service --loglevel info,cluster:debug,pintracker:debug daemon
`,
	programName,
	programName,
	DefaultFolder,
...
var (
	...
	DefaultFolder = ".rep-mgr"
	// DefaultPath is set on init() to $HOME/DefaultFolder
	// and holds all the rep-mgr data
	DefaultPath string
...
`,
					...
					Action: func(c *cli.Context) error {
						...
						opts := api.PinOptions{
							...
							UserAllocations:      strings.Split(c.String("allocations"), ","),
						}
...
`,
		...
		{
			Name:  "version",
			Usage: "Prints the rep-mgr version",
			Action: func(c *cli.Context) error {
				fmt.Printf("%s\n", version.Version)
				return nil
			},
